...
document.addEventListener('DOMContentLoaded', function(){
    ...
    document.querySelectorAll('.btn-ver').forEach(function(btn){
        btn.addEventListener('click', function(e){
            ...
            document.getElementById('v_categoria_experiencia').textContent = d.categoriaExperiencia || 'No especificada';
            ...
            // Antecedentes
            const antecedentesEl = document.getElementById('v_antecedentes');
            if(d.antecedentes && d.antecedentes.trim() !== ''){
                antecedentesEl.innerHTML = '<a href="' + d.antecedentes + '" target="_blank" style="color: #1976d2; text-decoration: underline;">📄 Ver documento</a>';
            } else {
                antecedentesEl.textContent = 'No adjuntado';
            }
            
            // Certificado Primeros Auxilios
            const primerosAuxiliosEl = document.getElementById('v_primeros_auxilios');
            if(d.primerosAuxilios && d.primerosAuxilios.trim() !== ''){
                primerosAuxiliosEl.innerHTML = '<a href="' + d.primerosAuxilios + '" target="_blank" style="color: #1976d2; text-decoration: underline;">📄 Ver documento</a>';
            } else {
                primerosAuxiliosEl.textContent = 'No adjuntado';
            }
            
            // Cargar experiencias desde el servidor
            const experienciasDetalleEl = document.getElementById('v_experiencias_detalle');
            experienciasDetalleEl.innerHTML = '<p style="text-align:center; color:#64748b;">Cargando experiencias...</p>';
            
            const idPersonaParaExperiencias = d.personapk || d.idpersona;
            const urlExperiencias = '/entrenadores/?obtener_experiencias=' + encodeURIComponent(idPersonaParaExperiencias);
            
            fetch(urlExperiencias, {
                method: 'GET',
                headers: {
                    'X-Requested-With': 'XMLHttpRequest'
                }
            ...
            .then(data => {
                if(data.success && data.experiencias && data.experiencias.length > 0){
                    ...
                    data.experiencias.forEach((exp, index) => {
                        html += '<div style="background:#f8fafc; padding:10px; border-radius:8px; border:1px solid #e2e8f0;">';
                        html += '<strong>Experiencia ' + (index + 1) + ':</strong> ' + exp.anios + ' años';
                        if(exp.certificado_url){
                            html += ' <a href="' + exp.certificado_url + '" target="_blank" style="color:#1976d2; text-decoration:underline; margin-left:10px;">📄 Ver certificado</a>';
                        }
                        html += '</div>';
                    });
                    experienciasDetalleEl.innerHTML = html;
                } else {
                    experienciasDetalleEl.innerHTML = '<p style="text-align:center; color:#64748b;">No hay experiencias registradas</p>';
                }
            })
            .catch(error => {
                console.error('Error cargando experiencias:', error);
                experienciasDetalleEl.innerHTML = '<p style="text-align:center; color:#dc3545;">Error al cargar experiencias</p>';
            });
            ...
            fetch('/entrenadores/?puntajes_postulantes=1', { headers: { 'X-Requested-With':'XMLHttpRequest' }})
                ...
                .then(data => {
                    ...
                    if(data.success){
                        ...
                        // Primero mostrar TODOS los postulantes
