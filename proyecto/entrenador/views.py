...
def entrenador(request):
    ...
    if request.method == 'GET':
        ...
        # Filtrar postulantes por categoría de experiencia
        if request.GET.get('filtrar_categoria') and is_ajax_generic:
            categoria = request.GET.get('filtrar_categoria')
            postulantes_filtrados = []
            
            if categoria:  # Si hay categoría específica
                postulantes_qs = PersonalT.objects.select_related('fk_persona').filter(
                    Q(postulante=True) | Q(estado_proceso='en_proceso'),
                    categoria_experiencia=categoria
                )
            else:  # Si es "Todas las categorías"
                postulantes_qs = PersonalT.objects.select_related('fk_persona').filter(
                    Q(postulante=True) | Q(estado_proceso='en_proceso')
                )
            
            for p in postulantes_qs:
                postulantes_filtrados.append({
                    'persona_pk': p.fk_persona.id,
                    'id_persona': p.fk_persona.id_persona,
                    'nombres': f"{p.fk_persona.nom1_persona} {p.fk_persona.nom2_persona or ''}".strip(),
                    'apellidos': f"{p.fk_persona.ape1_persona} {p.fk_persona.ape2_persona or ''}".strip(),
                    'email': p.fk_persona.email_persona,
                    'estado_proceso': p.estado_proceso,
                    'categoria_experiencia': p.categoria_experiencia or 'Sin especificar',
                    'experiencia': p.experiencia or 0,
                })
            
            return JsonResponse({
                'success': True, 
                'postulantes': postulantes_filtrados,
                'total': len(postulantes_filtrados),
    ...
    # Endpoint AJAX para obtener experiencias de un postulante
    if request.method == 'GET' and request.GET.get('obtener_experiencias'):
        is_ajax = request.headers.get('X-Requested-With') == 'XMLHttpRequest'
        if is_ajax:
            from partida.models import Experiencia
            persona_pk = request.GET.get('obtener_experiencias')
            try:
                experiencias = Experiencia.objects.filter(
                    fk_persona_experiencia_id=persona_pk
                ).order_by('idexperiencia')
                
                experiencias_list = []
                for exp in experiencias:
                    experiencias_list.append({
                        'anios': exp.anios_experiencia,
                        'certificado_url': exp.certificado_experiencia.url if exp.certificado_experiencia else None
                    })
                
                return JsonResponse({'success': True, 'experiencias': experiencias_list})
            except Exception as e:
                import traceback
                print(f"DEBUG ERROR experiencias: {str(e)}")
                print(traceback.format_exc())
                return JsonResponse({'success': False, 'message': f'Error: {str(e)}'})
    ...
    if request.method == 'POST':
        ...
        # Manejo de envío de correos personalizados/automáticos
        accion_correo = request.POST.get('accion_correo')
        if accion_correo and request.POST.get('id_postulante'):
            id_persona = request.POST.get('id_postulante')
            tipo_correo = request.POST.get('tipo_correo', 'automatico')
            ...
            if tipo_correo == 'personalizado':
                asunto = request.POST.get('asunto', '')
                mensaje = request.POST.get('mensaje', '')
                html_message = f"<html><body><p>{mensaje.replace(chr(10), '<br>')}</p></body></html>"
                plain_message = mensaje
            else:
                # Correo automático según acción
                ...
                if accion_correo in ['aceptar_final', 'aceptar_proceso']:
                    asunto = '¡Bienvenido al Equipo! - Club Deportivo Atlético Santander'
                    ...
                    asunto = 'Resultado de tu Postulación - Club Deportivo Atlético Santander'
                    html_message = render_to_string('emails/postulante_rechazado.html', {
                        'nombre_completo': nombre_completo,
                    })
                else:  # pasar_proceso
                    asunto = 'Invitación a Entrevista - Club Deportivo Atlético Santander'
                    html_message = f"""
                    <html><body>
                    <h2>Hola {nombre_completo},</h2>
                    <p>Nos complace informarte que has pasado a la siguiente etapa del proceso de selección.</p>
                    <p>Pronto nos pondremos en contacto contigo para coordinar una entrevista.</p>
                    <p>Saludos,<br>Club Deportivo Atlético Santander</p>
                    </body></html>
                    """
        ...
        if accion_post and request.POST.get('id'):
            ...
            # Rechazo directo desde postulante (sin modal, correo automático)
            if accion_post == 'rechazar_directo':
                personalt.postulante = False
                personalt.estado = False
                personalt.tipo_personal = ''
                personalt.estado_proceso = 'rechazado'
